...
namespace theme_boost_union_child\task;

/**
 * A scheduled task to add academic year custom field to courses.
 * /usr/bin/php admin/cli/scheduled_task.php --execute=\\theme_boost_union_child\\task\\add_academicyear_custom_field
 */
class add_academicyear_custom_field extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('academicyeartask', 'theme_boost_union_child');
    }
    ...
    public function execute() {
        global $DB;
        $yearfrom = get_config('theme_boost_union_child', 'includeaccyearsfrom') ?: date('y') - 1;
        $yearto = get_config('theme_boost_union_child', 'includeaccyearsto') ?: date('y') + 1;
        $customfieldconfig = get_config('theme_boost_union_child', 'customfield') ?: 'academicyear';
